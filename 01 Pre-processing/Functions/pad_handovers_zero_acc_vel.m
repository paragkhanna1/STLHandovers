function [handover_set] = pad_handovers_zero_acc_vel(handover_set)
%Padd signals. Acceleration and velocity are zero padded while position
    ...
    for handover_idx = 1:handover_set(set_idx).N_handovers
        ...
        for frame = frames
            ...
            for key = keys
                    ...
                    try
                        handover_set(set_idx).handover(handover_idx).signals.(frame).(key).type ;
                    catch
                        asdf=0
                    end
                    if (handover_set(set_idx).handover(handover_idx).signals.(frame).(key).type == "velocity") || (handover_set(set_idx).handover(handover_idx).signals.(frame).(key).type == "acceleration")
                        for dim = [1:size(data,2)]
                            data(len0+1 : longest_handover, dim) = zeros(longest_handover-len0, 1);
                        end
                        handover_set(set_idx).handover(handover_idx).signals.(frame).(key).data = data;
                    else % handover_set(set_idx).handover(handover_idx).signals.(frame).(key).type == "position"
                        for dim = [1:size(data,2)]
