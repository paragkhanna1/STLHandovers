...
class generate_signal_problem:
    ...
    def __init__(self, phi, start, domain, dimensions, U, dt, OPTIMIZE_ROBUSTNESS = True, QUANTITATIVE_OPTIMIZATION = True, HARD_STL_CONSTRAINT = False, KSHIRSAGAR_APPROACH = True, KSHIRSAGAR_DELTA = [0.3, 0.1, 0.0], MINIMIZE_MAX_VELOCITY = False, MINIMIZE_AVG_VELOCITY = False , Kshirsagar_phi = None, HARD_KSHIRSAGAR_CONSTRAINT = False, MINIMIZE_AVG_ACC = False):
        """
            Provides methods for generating a signal satisfying an STL Formula.
            Takes as input:
                * phi:      STL Formula; handover STL-model without the Kshirsagar part, see STL.py for syntax
                * start:    (NOT USED) a vector of the form [x0,y0,...] for the starting point coordinates
                * domain:   List of float; the domain in which signals are generated. self.domain = [lb,ub] where lb is the lower bound and ub the upper bound of the domain.
                * dimensions:   List of str; the dimensions on which the STLFormula is defined
                            Should be defined as    dimensions = ['giver_x',        'giver_y',      'giver_z',
                                                                  ...
                                                                  'K_p']
                * U:    List of float;  a basic control policy defining maximum speed and acceleration of giver and taker, limiting the states evolution over time. i.e. \forall t \in [0,T], |s[t]-s[t+1]| < U
                ...
                * dt:   float;  sampling time of system
            Optional inputs:
                * OPTIMIZE_ROBUSTNESS:      Bool; Indicates the robustness of the generated signal w.r.t. phi is maximized or not. i.e. robustness is a component of the objective function
                * QUANTITATIVE_OPTIMIZATION:Bool; Choose Quantitative or Qualitative optimization. Must be True! Qualitative optimization not implemented.
                * HARD_STL_CONSTRAINT:      Bool; Indicates STL robustness MUST be positive, or if negative robustness permitted. i.e. True -> robustness >= 0 is a constraint in the optimisation problem.
                * KSHIRSAGAR_APPROACH:      Bool; a flag whether phi contains the l1 distance norm, 'K_p' between giver and taker based on the "towards human hand" approach method in a paper by Kshirsagar et. al
                * Kshirsagar_phi:           STL Formula; Formula for Kshirsagar approach.
                * HARD_KSHIRSAGAR_CONSTRAINT:Bool; Indicates STL robustness of Kshirsagar phi must be positive for all plans.
                * KSHIRSAGAR_DELTA = [0.3, 0.1, 0.0]
                * MINIMIZE_AVG_VELOCITY:    Bool; a flag whether average velocity should be part of the objective function, computed as sum of l1 norm of all timesteps.
                * MINIMIZE_AVG_ACC:         Bool; Same as above, for acceleration.
                * MINIMIZE_MAX_VELOCITY:    Bool; a flag whether maximum velocity should be part of the objective function, computed as maximum l1 norm.

            The encoding details of the MILP optimization problem is based on the quantitative enconding of Raman et al., "Model  predictive  control  with  signaltemporal logic specifications" in 53rd IEEE Conference on Decision and Control. IEEE, 2014, pp. 81–87.
                with temporal operators evaluated for future signals, following my thesis
        """ 
        #CONSTANTS
        self.M = 100000
        self.M_up = 100000
        ...
        self.MINIMIZE_AVG_ACC = MINIMIZE_AVG_ACC
    ...
    def fix_variables(self, t, variable_idx, value=False):
        """
            Sets variables in the state vector (self.s). Used to encode known states (measured/past position) into the optimization problem.
    ...
    def unfix_variables(self, t, variable_idx):
        """
            Unfixes variables in state vector (self.s), so value can be optimized or changed.
            Inputs:
                t:              List of int Time index for all variables to set
                variable_idx:   List of int 2nd index of variables to set
        """
    ...
    def get_max_acceleration(self, times, idxs):
        """
            Calculates the maximum acceleration from possition values in state vector (self.s). 
            For trubleshooting, comparing calculated max acceleration to encoded acceleration.
            Inputs:
                times:              List of int;    Time index to search 
                idxs:               List of int;    2nd index on s, corresponding to possition variables
            Output:
               max_accelerations:   List of float;  Max acceleration over all given variables, for each time
        """
        max_accelerations = []
        max_speed = []
        try:
    ...
    def generate_path(self, current_step):
        """
            ...
            Returns self.s: The optimized state vector 
        """
    ...
    def get_path(self):
        """
            Returns the path, state vector as a nested list
        """
        return [[self.s[j][i].varValue for i in range(len(self.dimensions))] for j in range(self.phi.horizon+1)]
    ...
    def init_quantitave_problem(self):
        """
            ...
            The encoding details of the MILP optimization problem based on the quantitative enconding of Raman et al., "Model  predictive  control  with  signaltemporal logic specifications" in 53rd IEEE Conference on Decision and Control. IEEE, 2014, pp. 81–87.
        """    
        ...
        if self.MINIMIZE_AVG_VELOCITY:
            ...
            abs_velocity = plp.LpVariable.dicts("abs_velocity",(range(self.phi.horizon+1),range(6)), lowBound=0, cat=plp.LpContinuous)
            for eval_t in range(0,self.phi.horizon):
                for t in range(0,self.phi.horizon+1):
                    for i in [0,1,2]:
                        # |x| := max{x, -x}
                        # -t <= x <= t models |x|<=t
                        self.model_dict[eval_t] += -abs_velocity[t][i]                              <= self.s[t][i + self.dimensions.index('giver_dx')]
                        self.model_dict[eval_t] += self.s[t][i + self.dimensions.index('giver_dx')] <= abs_velocity[t][i]
                        self.model_dict[eval_t] += -abs_velocity[t][i+3]                            <= self.s[t][i + self.dimensions.index('taker_dx')]
                        self.model_dict[eval_t] += self.s[t][i + self.dimensions.index('taker_dx')] <= abs_velocity[t][i+3]
                    # |s| <= abs_velocity
                #objective_function = objective_function + plp.lpSum(abs_velocity)
                self.model_dict[eval_t] += plp.lpSum(abs_velocity)
    
        if self.MINIMIZE_AVG_ACC:
            # minimize absolute velocity at all times
            abs_accel = plp.LpVariable.dicts("abs_accel",(range(self.phi.horizon+1),range(6)), lowBound=0, cat=plp.LpContinuous)
            for eval_t in range(0,self.phi.horizon):
                for t in range(0,self.phi.horizon+1):
                    for i in [0,1,2]:
                        # |x| := max{x, -x}
                        # -t <= x <= t models |x|<=t
                        self.model_dict[eval_t] += -abs_accel[t][i]                                  <= self.s[t][i + self.dimensions.index('giver_ddx')]
                        self.model_dict[eval_t] += self.s[t][i + self.dimensions.index('giver_ddx')] <= abs_accel[t][i]
                        self.model_dict[eval_t] += -abs_accel[t][i+3]                                <= self.s[t][i + self.dimensions.index('taker_ddx')]
                        self.model_dict[eval_t] += self.s[t][i + self.dimensions.index('taker_ddx')] <= abs_accel[t][i+3]
                    # |s| <= abs_accel
                #objective_function = objective_function + plp.lpSum(abs_accel)
                self.model_dict[eval_t] += plp.lpSum(abs_accel)
...
def plot_trajectory2(phi, current_step, trajectory, dt, robot_role = 'Giver', giver_mse = None, taker_mse = None, t_start_mse = None, save_flag = False, close_flag = True, title_txt = None , true_human_x=False, true_human_y=False, true_human_zG=False, true_human_zT=False):
    ...
    '''
    print("plot_trajectory2 in STLGenerateJointHandover.py may not function correctly. I haven't tested it much.")
    ...
    x_giver = [x[dimensions.index('giver_x')] for x in trajectory]
    y_giver = [x[dimensions.index('giver_y')] for x in trajectory]
    z_giver = [x[dimensions.index('giver_z')] for x in trajectory]
    x_taker = [x[dimensions.index('taker_x')] for x in trajectory]
    y_taker = [x[dimensions.index('taker_y')] for x in trajectory]
    z_taker = [x[dimensions.index('taker_z')] for x in trajectory]
...
def plot_trajectory(phi, current_step, trajectory, human_data, dt, giver_mse = None, taker_mse = None, t_start_mse = None, save_flag = False, close_flag = True, title_txt = None ):
    print("Don't use this function. plot_trajectory was replaced by plot_trajectory2.")
...
if __name__ == '__main__':
    ...
    print("This Demo does not work! It was not updated with the rest of the code. Run one of the tests instead!")
    quit()
    ...
    PLOT_PATH = 'G:/My Drive/Exjobb Drive folder/'
