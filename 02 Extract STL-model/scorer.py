...
BETA = 80 # Beta >= 1, tightness of dimensional constraints, higher = tighter
#GAMMA = 0.01 # Gamma >= 0, tradeoff between time and dimensional constraints
GAMMA = 0.01
def set_beta_gamma(beta, gamma):
    global BETA
    global GAMMA
...
def _(stl, x, t):
    ...
    # Added by Jonathan
    if left>right:
        #print("Flag @qualitativescore.register(Future)")
        raise ValueError("Interval [{},{}] empty for {}".format(left, right, stl))    
...
def _(stl, x, t):
    ...
    # Added by Jonathan
    if left>right:
        #print(left, right, "Global q score **** ")
        raise ValueError("Interval [{},{}] empty for {}".format(left, right, stl)) 
...
def _(stl, x, t):
    ...
    # Added by Jonathan
    #print(t, left, right, "@quantitativescore.register(Future)")
    #if left>right:
        #print(left, right, "Future (eventually) q score **** ")
        #raise ValueError("Interval [{},{}] empty for {}".format(left, right, stl)) 
...
def _(stl, x, t):
    ...
    return  2/(1 + math.exp(-1 * GAMMA * intervalwidth) ) * min(smartscore(stl.subformula, x, t1) for t1 in rangetime)
...
def _(stl, x, t):
    ...
    right = float(right) 
    #if left>right:
        #print(left, right, " @smartscore.register(Future)")
        #raise ValueError("Interval [{},{}] empty for {}".format(left, right, stl)) 

    intervalwidth = right - left + 1
    (maxtime, rangetime) = gettime(x, t+left, t+right)
    return  2/(1 + math.exp(GAMMA * intervalwidth) ) * max(smartscore(stl.subformula, x, t1) for t1 in rangetime)
...
def _(stl, x, t):
    ...
    #    print(filter(lambda v: (v>= t) & (v<= t1) , rangetime) )
    #    print(min (qualitativescore(stl.left, x, t2) for t2 in filter(lambda v: (v>= t) & (v< t1) , rangetime) ) )
    return 2/(1 + math.exp(-1 * GAMMA * intervalwidth) ) * max(quantitativescore(stl.right, x, t), max( min (quantitativescore(stl.right, x, t1), min (quantitativescore(stl.left, x, t2) for t2 in filter(lambda v: (v>= t) & (v<= t1) , rangetime) ) ) for t1 in rangetime) )
...
def _(stl, x, t):
    ...
    #print(BETA)
    return 1/(rawscore + math.exp(-1*BETA*rawscore)) - math.exp(-1*rawscore) 
    #else :
