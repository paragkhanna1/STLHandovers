...
import stl as stl
import parametrizer as parametrizer
import scorer as scorer
import inputreader as inputreader
#import bayesopt
import numpy as np
...
# If on py >3.3, use monotonic, if older use clock
if (sys.version_info[0] == 3 and sys.version_info[1] < 3) or sys.version_info[0] < 3:
    from time import clock
    use_clock = True
else:
    from time import monotonic
...
def simoptimize(stl, tracelist,scorefun=scorer.smartscore,optmethod='HYBRID', tol = 1e-1):
    ...
    if use_clock:
        ...
        return (pvalue, mvalue, monotonic()-start)
...
def bayesoptimize(stl, tracelist, iter_learn, iter_relearn, init_samples, mode, steps=10, NumAttempts = 10):
    ...
    if use_clock:
        ...
        return (x_out, mvalue, monotonic()-start)
...
def synthSTLParam(tlStr, tracedir, optmethod="gradient", tol = 1e-1):
    ...
    # Added by Jonathan
    ppvalue = pvalue # Skip post processing
...
def synthSTLParam2(tlStr, tracedir, optmethod="gradient", tol = 1e-1):
    # Copy with aditional output for values
    stlex = stl.parse(tlStr)
    param = parametrizer.getParams(stlex)
    logging.debug("\nTo Synthesize STL Template: {}".format(stlex))
    tracenamelist = find_filenames (tracedir, suffix=".csv")
    ...
    logging.debug("\ndef synthSTLParam2")
    #logging.debug( "stlex = {}".format(stlex))
    logging.debug("ppvalue = {}".format(ppvalue))

    logging.debug("Opt method used: {}".format(optmethod))
    logging.debug("Synthesized STL: {}".format(stlsyn))
    logging.debug("Synthesis: Cost is {}, Time taken is {}".format(value, dur))
    return stlsyn, -1*value, dur, ppvalue #we were minimizing negative of theta
